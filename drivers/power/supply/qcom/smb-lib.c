...
#define SMBCHG_FAST_CHG_CURRENT_VALUE_2100MA	0x54
...
static int SW_recharge(struct smb_charger *chg)
{
	...
	pr_info("%s: bat_capacity = %d, termination_reg = 0x%x\n", __func__,
		capacity, termination_reg);

	if (capacity <= 98 && termination_done) {
		pr_info("will start %s\n", __func__);
		rc = smblib_masked_write(chg, CHARGING_ENABLE_CMD_REG,
					 CHARGING_ENABLE_CMD_BIT,
					 CHARGING_ENABLE_CMD_BIT);
		if (rc < 0) {
...
int smbchg_jeita_judge_state(int old_State, int batt_tempr)
{
	...
	//decide value to set each reg (Vchg, Charging enable,
	//Fast charge current)
	//batt_tempr < 0
	if (batt_tempr < 0) {
...
void jeita_rule(void)
{
	...
	case JEITA_STATE_RANGE_100_to_500:
		charging_enable = EN_BAT_CHG_EN_COMMAND_TRUE;
		FV_CFG_reg_value = SMBCHG_FLOAT_VOLTAGE_VALUE_4P350;
		FCC_reg_value = SMBCHG_FAST_CHG_CURRENT_VALUE_2100MA;
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	pr_info("%s: enter micro_usb_mode\n", __func__);
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	if (!chg->typec_legacy_valid) {
		...
		pr_info("%s: smblib_force_legacy_icl has done\n", __func__);
	}
	switch (apsd_result->bit) {
	case SDP_CHARGER_BIT:
	case CDP_CHARGER_BIT:
