...
static int32_t nvt_read_CC(int32_t *xdata)
{
	...
	if (ts->carrier_system) {
		...
		if (nvt_save_rawdata_to_csv(RawData_FW_CC_Q, X_Channel,
					    Y_Channel, FW_CC_CSV_FILE,
					    rawdata_cc_q_offset) < 0) {
			pr_err("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
	} else {
...
static int32_t nvt_read_fw_noise(int32_t *xdata)
{
	...
	for (y = 0; y < y_num; y++) {
		for (x = 0; x < x_num; x++) {
			...
			if (ts->carrier_system) {
				...
				RawData_Diff_Max[iArrayIndex] = (int8_t)(
					(xdata[iArrayIndex] >> 8) & 0xFF);
	...
	if (!ts->carrier_system) {
#if TOUCH_KEY_NUM > 0
		rawdata_diff_min_offset = Y_Channel * X_Channel * 7 +
					  Y_Channel * 2 + Key_Channel * 7 + 2;
#else
...
static int32_t nvt_read_fw_open(int32_t *xdata)
{
	...
	for (y = 0; y < IC_Y_CFG_SIZE; y++) {
		...
		buf[0] = 0xFF;
		buf[1] = (uint8_t)(
			((raw_pipe_addr + y * IC_X_CFG_SIZE * 2) >> 16) & 0xFF);
		buf[2] = (uint8_t)(
			((raw_pipe_addr + y * IC_X_CFG_SIZE * 2) >> 8) & 0xFF);
		CTP_I2C_WRITE(ts->client, I2C_FW_Address, buf, 3);
	...
	for (k = 0; k < IC_KEY_CFG_SIZE; k++) {
		if (AIN_KEY[k] != 0xFF)
			xdata[Y_Channel * X_Channel + AIN_KEY[k]] = (int16_t)(
				rawdata_buf[(IC_Y_CFG_SIZE * IC_X_CFG_SIZE + k) *
					    2] +
				256 * rawdata_buf[(IC_Y_CFG_SIZE * IC_X_CFG_SIZE +
						   k) * 2 +
						  1]);
	}
#endif /* #if TOUCH_KEY_NUM > 0 */
...
static int32_t nvt_read_fw_short(int32_t *xdata)
{
	...
	for (y = 0; y < Y_Channel; y++) {
		for (x = 0; x < X_Channel; x++) {
			...
			xdata[iArrayIndex] = (int16_t)(
				rawdata_buf[iArrayIndex * 2] +
	...
	if (ts->carrier_system) {
		...
		for (y = 0; y < Y_Channel; y++) {
			...
			buf[2] = (uint8_t)(
				...
				0xFF);
			CTP_I2C_WRITE(ts->client, I2C_FW_Address, buf, 3);
			buf[0] = (uint8_t)((raw_pipe_addr + y * X_Channel * 2) &
					   0xFF);
			CTP_I2C_READ(ts->client, I2C_FW_Address, buf,
				     X_Channel * 2 + 1);
			memcpy(rawdata_buf + y * X_Channel * 2, buf + 1,
			       X_Channel * 2);
		}
#if TOUCH_KEY_NUM > 0
		...
		for (y = 0; y < Y_Channel; y++) {
			for (x = 0; x < X_Channel; x++) {
				iArrayIndex = y * X_Channel + x;
				RawData_Short_Base[iArrayIndex] = (int16_t)(
					rawdata_buf[iArrayIndex * 2] +
					256 * rawdata_buf[iArrayIndex * 2 + 1]);
			}
		}
	...
	if (ts->carrier_system) {
		...
		// Save Rawdata to CSV file
		if (nvt_save_rawdata_to_csv(RawData_Short_Base, X_Channel,
					    Y_Channel, SHORT_TEST_CSV_FILE,
...
static int32_t RawDataTest_Sub(int32_t rawdata[], uint8_t RecordResult[],
			       ...
			       int32_t Rawdata_Limit_Key_Postive,
			       int32_t Rawdata_Limit_Key_Negative)
{
	int32_t i = 0;
	int32_t j = 0;
#if TOUCH_KEY_NUM > 0
			...
			if (ts->carrier_system) {
				if (TestResult_FW_CC_I == -1) {
					...
					print_selftest_result(
						m, TestResult_FW_CC_I,
						RecordResult_FW_CC_I,
						RawData_FW_CC_I, X_Channel,
						Y_Channel);
				}
				if (TestResult_FW_CC_Q == -1) {
					nvt_mp_seq_printf(m, "FW CC_Q");
					print_selftest_result(
						m, TestResult_FW_CC_Q,
						RecordResult_FW_CC_Q,
						RawData_FW_CC_Q, X_Channel,
						Y_Channel);
				}
			} else {
...
// Huaqin add for esd check function. by zhengwu.lu. at 2018/2/28  start
	...
	if (nvt_read_CC(RawData_FW_CC) != 0) {
		...
		if (ts->carrier_system) {
			TestResult_FW_CC_I = RawDataTest_Sub(
				RawData_FW_CC_I, RecordResult_FW_CC_I,
				X_Channel, Y_Channel, PS_Config_Lmt_FW_CC_I_P,
				PS_Config_Lmt_FW_CC_I_N,
				PS_Config_Lmt_Key_FW_CC_I_P,
				PS_Config_Lmt_Key_FW_CC_I_N);
			TestResult_FW_CC_Q = RawDataTest_Sub(
				RawData_FW_CC_Q, RecordResult_FW_CC_Q,
				X_Channel, Y_Channel, PS_Config_Lmt_FW_CC_Q_P,
				PS_Config_Lmt_FW_CC_Q_N,
				PS_Config_Lmt_Key_FW_CC_Q_P,
				PS_Config_Lmt_Key_FW_CC_Q_N);
			if ((TestResult_FW_CC_I == -1) ||
			...
			TestResult_FW_CC = RawDataTest_Sub(
				RawData_FW_CC, RecordResult_FW_CC, X_Channel,
	...
	if (nvt_read_fw_noise(RawData_Diff) != 0) {
		...
		TestResult_FW_DiffMax = RawDataTest_Sub(
			RawData_Diff_Max, RecordResult_FW_DiffMax, X_Channel,
			Y_Channel, PS_Config_Lmt_FW_Diff_P,
			PS_Config_Lmt_FW_Diff_N, PS_Config_Lmt_Key_FW_Diff_P,
			PS_Config_Lmt_Key_FW_Diff_N);
		...
		if (ts->carrier_system) {
			...
			TestResult_FW_DiffMin = RawDataTest_Sub(
				RawData_Diff_Min, RecordResult_FW_DiffMin,
	...
	if (nvt_read_fw_short(RawData_Short) != 0) {
		...
		if (ts->carrier_system) {
			...
			TestResult_Short = RawDataTest_Sub(
				RawData_Short, RecordResult_Short, X_Channel,
	...
	if (TestResult_FWMutual == 0 && TestResult_Short == 0 &&
	    TestResult_Open == 0) {
		nvt_TestResultLen = 1;
	} else {
		nvt_TestResultLen = 0;
...
return:
	n.a.
*******************************************************/
void nvt_mp_parse_ain(struct device_node *np, const char *name, uint8_t *array,
		      int32_t size)
{
