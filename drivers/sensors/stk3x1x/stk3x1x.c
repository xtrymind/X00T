...
static int32_t stk3x1x_check_pid(struct stk3x1x_data *ps_data)
{
	...
	pr_debug("%s: p_wv_r_bd_with_co = 0x%x\n", __func__,
	       ps_data->p_wv_r_bd_with_co);

	if (otp25 & 0x40)
		ps_data->p_1x_r_bd_with_co |= 0b001;
	pr_debug("%s: p_1x_r_bd_with_co = 0x%x\n", __func__,
	       ps_data->p_1x_r_bd_with_co);

	pr_debug("%s: p_19_r_bc = 0x%x\n", __func__,
	       ps_data->p_19_r_bc);

	if (value[0] == 0) {
		pr_err("PID=0x0, please make sure the chip is stk3x1x!\n");
		return -2;
	}
...
*/
	...
	__pm_wakeup_event(&ps_data->ps_wakelock, jiffies_to_msecs(3 * HZ));
}
				  ...
				  uint8_t mode)
{
	...
	if (ps_data->ps_enabled) {
		pr_debug("%s: since PS is enabled, ges is disabled\n",
		       __func__);
...
/* Huaqin add for ZQL1650-1072 by zhuqiang at 2018/04/23 end*/
	...
	if (validate_reg) {
		...
		if (ret < 0)
			pr_err("stk3x1x_validate_n_handle fail: %d\n",
			       ret);
	}
...
static int32_t stk3x1x_enable_als(struct stk3x1x_data *ps_data, uint8_t enable)
{
	...
	if (ps_data->ges_enabled) {
		pr_debug("%s: since ges is enabled, ALS is disabled\n",
		       __func__);
	...
	w_state_reg = (uint8_t)(
		ret & (~(STK_STATE_EN_ALS_MASK | STK_STATE_EN_WAIT_MASK)));
	if (enable)
...
#endif
			// pr_debug("%s: als_data=%d, als_code_last=%d,ir_data=%d\n",
...
#ifdef STK_DEBUG_PRINTF
		pr_debug("%s: als=%d, ir=%d, als_correct_factor=%d",
				      ...
				      int32_t als_it_reduce)
{
	...
	if (retry == 10) {
		pr_err("%s: ir data is not ready for a long time\n",
		       __func__);
		ret = -EINVAL;
		goto irs_err_i2c_rw;
	}
...
#endif
	{
		pr_err("%s: invalid reg 0x02=0x%2x\n", __func__,
		       value[1]);
		return 0xFF;
	}
...
static int stk3x1x_validate_n_handle(struct i2c_client *client)
{
	...
	if (err < 0) {
		pr_err("stk3x1x_chk_reg_valid fail: %d\n", err);
		return err;
	}
				  ...
				  const char *buf, size_t size)
{
	...
	switch (value) {
	case 3:
		//pr_debug("%s: ges input event, not detected\n",__func__);
	case 0:
		return size;
	case 1:
		ges = KEY_PAGEUP;
		atomic_set(&ps_data->gesture2, 0);
		pr_debug("%s: ges input event >>>\n", __func__);
		break;
	case 2:
		ges = KEY_PAGEDOWN;
		atomic_set(&ps_data->gesture2, 0);
	...
	case 32:
		ges = KEY_VOLUMEDOWN;
		pr_debug("%s: ges input event near\n", __func__);
		break;
	case 48:
		ges = KEY_VOLUMEUP;
		pr_debug("%s: ges input event far\n", __func__);
		break;
	default:
		pr_err("%s, invalid value %d\n", __func__, (int)value);
		return -EINVAL;
			      ...
			      const char *buf, size_t size)
{
	...
	if (0 != ret) {
		pr_err("%s: stk3x1x_i2c_smbus_write_byte_data fail\n",
		       __func__);
		return ret;
	}
...
static int stk_ps_val(struct stk3x1x_data *ps_data)
{
	...
	if (mode == 0x30)
		...
		pr_err("%s: unsupported PS_IT(0x%x)\n", __func__,
		       mode);
		return -1;
	}

	if (word_data > lii) {
		pr_debug("%s: word_data=%d, lii=%d\n", __func__,
		       word_data, lii);
		return 0xFFFF;
	}
	return 0;
	...
	if ((ps_data->psi_set != 0)) {
		...
		if (ret == 0) {
			ret = stk3x1x_i2c_read_data(ps_data->client, 0x11, 2,
						    &value[0]);
			if (ret < 0) {
				pr_err("%s fail, err=0x%x", __func__,
				       ret);
				return ret;
			}
			word_data = (value[0] << 8) | value[1];
			if (word_data > 0) {
				...
				/*Huaqin modify for psensor near/far threshold by chenyijun5 at 2018/02/27 end*/
				{
					...
					stk3x1x_set_ps_thd_l(ps_data,
							     ps_data->ps_thd_l);
					pr_debug("%s: CTTRACKING set HT=%d, LT=%d\n",
					       __func__, ps_data->ps_thd_h,
					       ps_data->ps_thd_l);
					ps_data->psi = word_data;
					pr_debug("%s: CTTRACKING update psi=%d\n",
...
static int stk_tune1_ct_det(struct stk3x1x_data *ps_data, uint32_t ps)
{
	...
	if (err == 0xFFFF) {
		if (ps_data->invalid_ps_cnt++ > STK_TUNE1_INVALID_PS_LIMIT) {
			pr_debug("%s: invalid_ps_cnt > limit\n", __func__);
			return 3;
		}
		return 0;
	}
	...
	if (ct >= 0) {
		if (ct < STK_PS_CT_LIMIT) {
			...
			pr_debug("%s: ct>STK_PS_CT_LIMIT,keep HT/LT\n", __func__);
			return 2;
		}
	}
	return 0;
...
*/
		...
		input_sync(ps_data->als_input_dev);
#ifdef STK_DEBUG_PRINTF
		pr_debug("%s: ges_enabled=1, als input event %d lux\n",
				   ...
				   uint32_t ps_reading)
{
	...
	pr_debug("%s:handle_state=%d,ps=%d,als=%d,nf_flag=%d,ps_distance_last=%d\n",
	...
	case STK_PSINT_CT_DET:
		...
		break;
	default:
		pr_err("%s: invalid stk_int_handle_state %d\n",
	...
	if (ret == 1) {
		...
		pr_debug("%s: HT=0,LT=0xffff\n", __func__);

		stk_tune1_reset_para();
		pr_debug("%s: reset ps_get_ct\n", __func__);
	}
	return 0;
}
...
/*Huaqin add for ps INT mode by chenyijun5 at 2018/03/29 start*/
		...
		if (near_far_state == 0) {
			if ((reading > (ps_data->psi + STK_H_PS)) &&
				...
				err = stk3x1x_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
				if (err) {
					pr_err("write high thd error: %d\n",
					       err);
				}

				err = stk3x1x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);
				if (err) {
					pr_err("write high thd error: %d\n",
					       err);
				}
				...
				pr_debug("%s:ps update ps = 0x%x, psi = 0x%x\n",
...
static void stk3x1x_als_set_poll_delay(struct stk3x1x_data *ps_data)
{
	...
	if (als_it == 0x8) {
		...
		ps_data->als_poll_delay = ns_to_ktime(110 * NSEC_PER_MSEC);
		pr_debug("%s: unknown ALS_IT=%d, set als_poll_delay=110ms\n",
		       __func__, als_it);
	}
}
#endif
...
static int stk3x1x_power_ctl(struct stk3x1x_data *data, bool on)
{
	...
	if (!on && data->power_enabled) {
		...
		pr_debug("%s: disable stk3x1x power", __func__);
		...
		data->power_enabled = on;
		pr_debug("%s: enable stk3x1x power", __func__);
		dev_dbg(&data->client->dev, "stk3x1x_power_ctl on=%d\n", on);
	} else {
...
static int stk3x1x_set_input_devices(struct stk3x1x_data *ps_data)
{
	...
	if (ps_data->ges_input_dev == NULL) {
		pr_err("%s: could not allocate ges device\n",
		       __func__);
		err = -ENOMEM;
	...
	*/
	...
	if (err < 0) {
		pr_err("%s: can not register ges input device\n",
		       __func__);
	...
	if (err < 0) {
		pr_err("%s:could not create sysfs group for ges\n",
		       __func__);
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (!ps_data) {
		pr_err("%s: failed to allocate stk3x1x_data\n",
		       __func__);
		return -ENOMEM;
	}
	...
	pr_info("%s: probe successfully", __func__);
	return 0;
...
#ifdef STK_POLL_PS
	wakeup_source_destroy(&ps_data->ps_nosuspend_wl);
#endif
	wakeup_source_destroy(&ps_data->ps_wakelock);
